...
class EntityFullTextRequiredTermsTest {
    ...
    @Test
    @DisplayName("EXPRESSION mode adds nothing to the entity query, but still escapes it")
    fun `EXPRESSION mode passes the entity query through untouched`() {
        ...
        // FORK DIVERGENCE (Le Maître): upstream binds the caller's string byte for byte here. This
        // fork escapes it first — see LuceneQuery.sanitizeForMode. Callers are agents that
        // concatenate conversation context (`[Recent Messages] ...`) rather than authors of Lucene
        // expressions, and an unescaped `[` is a ParseException that takes down all retrieval.
        // What the mode still guarantees is the part this test exists to pin: preparation adds
        // NOTHING of its own — no required-term operator, and so no second search.
